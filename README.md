...
I would also later adjust my audio preprocessing methods so that instead of feeding my neural network melspectrogram data, I would feed it autocorrelation function data. Autocorrelation functions are often used to find repeating patterns (i.e. the period) in a signal; I applied an autocorrelation function to each Mel in my previous melspectrograms. The hope was that this transformation would make it easier for my neural network to predict the tempo, since the input data was less complicated.
