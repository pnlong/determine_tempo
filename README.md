...
I will use my personal song library (>2000 songs) to train this neural network, excluding classical and other genres of music that tend to have variable tempos. I will create a data table that contains information on each song's BPM, as well as their key for later use. This will probably involve creating a webscraper that takes an .MP3 as input and uses the MP3's metadata to search a website like [MusicStax](https://https://musicstax.com/) for important song information. I will divide my data 70%-20%-10%: 70% training data, 20% cross validation data, and the remaining 10% for measuring actual performance metrics. I will use 10-second long melspectrogram data for training my neural network -- a couple convolutional layers should be able to pick up on patterns in the mel spectrograms, such as drumbeats or basslines. By chopping each song into 30 clips of audio (each 10-seconds long), I ended up training my neural network on 76,055 audio samples.

I would also later adjust my audio preprocessing methods so that instead of feeding my neural network melspectrogram data, I would feed it autocorrelation function data. Autocorrelation functions are often used to find repeating patterns (i.e. the period) in a signal; I applied an autocorrelation function to each Mel in my previous melspectrograms. The hope was that this transformation would make it easier for my neural network to predict the tempo, since the input data was less complicated. See more about this in the discussion of my results.


## Machine Learning

I will use PyTorch to create a Convolutional Neural Network (CNN) that takes **.wav** clips from **.mp3** files as input and outputs a single value, the song's tempo measured in BPM. This is a linear regression problem. The hope is that a CNN can better detect patterns in the audio data such as a kick drum or bassline that are useful in maintaining (and thus determining) a song's tempo.

I can use two approaches for this neural network:

- **Window Method**: Because the input of a CNN needs to be of a fixed size, I can train my network on 10-second (or any other amount of time) snippets of audio: *windows*. Using the same window size on which my neural network was trained, I would then scan over a song and for each window output a predicted tempo. The average song would contain about 30 windows (and thus predicted BPM values), so I would ultimately take the mode tempo from this list of BPMs for a final value. The benefit of this method is that there is potential for a lot of training data, as the average song can be divided up into, as mentioned previously, 30 samples.
...
Ultimately, I decided on the Window Method, using a 10 second window. A new window begins every 5 seconds. The trained neural network parameters can be found at [this link](https://drive.google.com/file/d/18D2ocW4iygeg4DA2c4j4yWzxTNa7p4Dg/view?usp=sharing).
